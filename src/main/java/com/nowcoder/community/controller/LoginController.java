...
public class LoginController implements CommunityConstant {
     ...
     *
     * @param response // @param session  用来存储 验证码 本身的值，所以用session 比较合适
     *                 <p>
     *                 change 使用 Redis 存放临时的验证码数据
     */
    @RequestMapping(path = "/kaptcha", method = RequestMethod.GET)
    public void getKaptcha(HttpServletResponse response/*, HttpSession session*/) {
        // 1. 生成验证码，在前面 KaptchaConfig 配置下的 kaptcha 工具，需要注入进来。
        ...
        //3. 验证码存储 Redis
        String kaptchaKey = RedisKeyUtils.getPrefixKaptcha(kaptchaOwner);
...
//    }
     ...
     * <p>
     * 修改 使用上面更改后的验证码方法，从 Redis 中获取验证码
     * 在 @CookieValue 参数中， required = false，默认为 true，若该参数不存在抛出异常，false 则不抛出异常
     */
    @RequestMapping(path = "/login", method = RequestMethod.POST)
    // @ResponseBody
    public String login(String username, String password, String code, boolean rememberMe,
                        Model model/*, HttpSession session*/, HttpServletRequest request, HttpServletResponse response,
                        @CookieValue(name = "kaptchaOwner") String kaptchaOwner) {
        // 1. 从 Redis 中根据验证码标记，kaptchaOwner(cookie 中) 获取验证码
        String kaptcha = null;
